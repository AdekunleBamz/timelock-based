...
/**
 * @title LockOptions V3
 * @notice Manages lock duration options for the TimelockVault
 * @dev Version 3.0 - Fresh deployment with 0% bonus rates
 */
contract LockOptionsV3 is Ownable {
     ...
     * @param optionId The ID of the lock option to update
    ...
    /**
     * @notice Deactivate a lock option
     * @param optionId The ID of the lock option to deactivate
     */
    function deactivateLockOption(uint8 optionId) external onlyOwner {
        require(optionId < lockOptions.length, "Invalid option ID");
        lockOptions[optionId].active = false;
    ...
    /**
     * @notice Activate a lock option
     * @param optionId The ID of the lock option to activate
     */
    function activateLockOption(uint8 optionId) external onlyOwner {
        require(optionId < lockOptions.length, "Invalid option ID");
        lockOptions[optionId].active = true;
        emit LockOptionUpdated(
            optionId,
            lockOptions[optionId].duration,
            lockOptions[optionId].bonusRate,
