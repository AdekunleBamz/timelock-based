...
/**
 * @title VaultRouter V2
 * @notice Helper contract for interacting with the TimelockVault
 * @dev Version 2.0 - Updated to work with TimelockVaultV2
 */

interface ITimelockVaultV2 {
...
contract VaultRouterV2 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    string public constant VERSION = "2.0.0";
    
    IERC20 public immutable usdc;
    ITimelockVaultV2 public vault;
     ...
     * @param lockOptionId Lock period option
     */
    ...
    function canWithdraw(uint256 depositId) external view returns (bool) {
        (,, uint256 lockEndTime, bool withdrawn,) = vault.getDeposit(depositId);
        return !withdrawn && block.timestamp >= lockEndTime;
    }
    ...
    function getTimeRemaining(uint256 depositId) external view returns (uint256) {
        (,, uint256 lockEndTime, bool withdrawn,) = vault.getDeposit(depositId);
        if (withdrawn || block.timestamp >= lockEndTime) {
            return 0;
        }
